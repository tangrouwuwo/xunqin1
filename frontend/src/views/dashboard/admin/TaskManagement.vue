  ...
  <div>
    ...
    <div class="row mb-3">
      ...
      <div class="col-md-4 d-flex justify-content-end">
        <div class="btn-group">
          <button class="btn" :class="activeTab === 'all' ? 'btn-primary' : 'btn-outline-primary'" @click="activeTab = 'all'; loadData()">所有任务</button>
    ...
    <div class="table-responsive">
      <table class="table table-hover">
        ...
        <tbody>
          ...
          <tr v-else-if="list.length === 0"><td colspan="10" class="text-center text-muted py-4">暂无任务</td></tr>
          <tr v-for="item in list" :key="item.id">
            ...
            <td>
              ...
              <button v-if="item.status === 4" class="btn btn-sm btn-outline-success me-1" @click="showApprove(item)"><i class="fas fa-check"></i></button>
              <button v-if="item.status === 4" class="btn btn-sm btn-outline-danger me-1" @click="showReject(item)"><i class="fas fa-times"></i></button>
              <button v-if="item.status !== 2 && item.status !== 3 && item.status !== 5" class="btn btn-sm btn-outline-primary me-1" @click="handleEdit(item)"><i class="fas fa-edit"></i></button>
      ...
      <div class="modal-dialog modal-lg">
        <div class="modal-content">
          <div class="modal-header">
            <h5 class="modal-title">{{ editingItem ? '编辑任务' : '创建任务' }}</h5>
          ...
          <div class="modal-footer">
            ...
            <button class="btn btn-primary" @click="submitTask" :disabled="!form.title">{{ editingItem ? '保存修改' : '发布任务' }}</button>
          </div>
        </div>
      </div>
    </div>
      ...
      <div class="modal-dialog modal-lg">
        <div class="modal-content">
          ...
          <div class="modal-body">
            <div class="row">
              <div class="col-md-6">
                <p><strong>发布者：</strong>{{ detailItem.publisherName }}</p>
    ...
    <div v-if="showReviewModal" class="modal d-block" tabindex="-1">
      <div class="modal-dialog">
        <div class="modal-content">
          <div class="modal-header">
            <h5 class="modal-title">{{ reviewAction === 'approve' ? '审核通过' : '拒绝任务' }}</h5>
            <button type="button" class="btn-close" @click="showReviewModal = false"></button>
          </div>
          <div class="modal-body">
            <p><strong>任务：</strong>{{ reviewTaskItem?.title }}</p>
            <p><strong>发布者：</strong>{{ reviewTaskItem?.publisherName }}</p>
            <div v-if="reviewAction === 'reject'" class="mb-3">
              <label class="form-label">拒绝原因 <span class="text-danger">*</span></label>
              <textarea v-model="reviewRemark" class="form-control" rows="3" placeholder="请输入拒绝原因"></textarea>
            </div>
            <p v-else class="text-success">确认通过该任务的审核？通过后任务将出现在任务大厅供志愿者认领。</p>
          </div>
          <div class="modal-footer">
            <button class="btn btn-secondary" @click="showReviewModal = false">取消</button>
            <button v-if="reviewAction === 'approve'" class="btn btn-success" @click="submitReview">确认通过</button>
            <button v-else class="btn btn-danger" @click="submitReview" :disabled="!reviewRemark">确认拒绝</button>
          </div>
        </div>
      </div>
    </div>
...
async function submitTask() {
  ...
  try {
    if (editingItem.value) {
      ...
      const res = await adminApi().createTask(data)
      if (res.code === 200) {
...
async function submitReview() {
  ...
  try {
    const res = await taskApi().review(reviewTaskItem.value.id, {
